...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchProfileData = async () => {
      ...
      try {
        setIsLoading(true)
        const data = await DatabaseService.getUserProfileData(user.uid)
        ...
        if (data) {
          ...
          if (storedProfile) {
            try {
              ...
              // Save to Firebase if we had local data
              await DatabaseService.saveUserProfileData(user.uid, parsed)
            } catch (error) {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Save to Firebase
      await DatabaseService.saveUserProfileData(user.uid, profileData)
        ...
        <main className="flex-1 flex items-center justify-center px-4 py-12">
          <div className="w-full max-w-3xl">
            <div className="bg-white dark:bg-stone-800 rounded-3xl shadow-xl border border-stone-100 dark:border-stone-700 overflow-hidden transition-colors duration-300">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-stone-600 dark:text-stone-400 leading-relaxed">
                      Your profile information is stored securely in our database. We use this information to assist you with document processing tasks.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {isLoading ? (
                  <div className="px-8 sm:px-10 pb-10 flex justify-center">
                    <div className="w-8 h-8 border-2 border-stone-300 border-t-stone-600 rounded-full animate-spin"></div>
                  </div>
              ) : (
                  <div className="px-8 sm:px-10 pb-10 space-y-8">
                    ...
                    <div className="pt-4">
                      ...
                      >
                        {isLoading ? (
                            <div className="w-4 h-4 border-2 border-white dark:border-stone-900 border-t-transparent dark:border-t-transparent rounded-full animate-spin mr-2"></div>
                        ) : (
